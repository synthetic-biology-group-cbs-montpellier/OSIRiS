...
def integrase_sites(int_list, directory_seq):
    ...
    for integrase in int_list:
        if 'attb_'+integrase.lower() in dico_seq:
            BF_site.append(dico_seq.get('attb_'+integrase.lower()))
        else:
            Error=True
        if Error!=True and 'attp_'+integrase.lower() in dico_seq:
            PF_site.append(dico_seq.get('attp_'+integrase.lower()))
        else:
            Error=True
        if Error!=True and 'attl_'+integrase.lower() in dico_seq:
            LF_site.append(dico_seq.get('attl_'+integrase.lower()))
        else:
            Error=True
        if Error!=True and 'attr_'+integrase.lower() in dico_seq:
    ...
    if end=='n':
        if type_rec=='inversion':
            if position_site1<position_site2:
                ...
                DNA_seq =str.replace(DNA_seq, DNA_seq[(position_site1+len(BF_site[int_num])):position_site2], DNA_seq[(position_site1+len(BF_site[int_num])):position_site2].translate(tab)[::-1])
                #DNA_seq[(position_site1+len(BF_site[int_num])):position_site2].reverse_complement()
                if new_site=='L':
                    DNA_seq =str.replace(DNA_seq, BF_site[int_num], LF_site[int_num])
                    DNA_seq=str.replace(DNA_seq, PR_site[int_num], RR_site[int_num])
                else:
                    DNA_seq =str.replace(DNA_seq, BR_site[int_num], RR_site[int_num])
                    DNA_seq=str.replace(DNA_seq, PF_site[int_num], LF_site[int_num])
            else:
                DNA_seq =str.replace(DNA_seq, DNA_seq[(position_site2+len(PF_site(int_num))):position_site1], DNA_seq[(position_site2+len(PF_site(int_num))):position_site1].translate(tab)[::-1])
                ...
                if new_site=='L':
                    DNA_seq =str.replace(DNA_seq, BF_site[int_num], RF_site[int_num])
                    DNA_seq=str.replace(DNA_seq, PR_site[int_num], LR_site[int_num])
                else:
                    DNA_seq =str.replace(DNA_seq, BR_site[int_num], LR_site[int_num])
                    DNA_seq=str.replace(DNA_seq, PF_site[int_num], RF_site[int_num])
...
# directory='/Users/Maczang/Documents/GitHub/3-OSIRiS'
